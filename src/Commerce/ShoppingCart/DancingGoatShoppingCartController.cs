...
public sealed class DancingGoatShoppingCartController : Controller
{
    ...
    public async Task<IActionResult> Index(CancellationToken cancellationToken)
    {
        ...
        decimal totalWithoutShippingAndTax = PriceCalculationTotalsCalculator.GetTotalWithoutShippingAndTax(calculationResult);
        decimal subtotal = PriceCalculationTotalsCalculator.GetSubtotal(calculationResult);
        decimal subtotalAfterLineDiscount = PriceCalculationTotalsCalculator.GetSubtotalAfterLineDiscount(calculationResult);
        decimal totalDiscount = PriceCalculationTotalsCalculator.GetTotalDiscountAmount(calculationResult);

        string orderDiscountText = await GetOrderDiscountInfoText(calculationResult, subtotalAfterLineDiscount, shoppingCartData, cancellationToken);
        ...
        return View(new ShoppingCartViewModel(
            shoppingCartData.Items.Select(item =>
            {
                ...
                productPageUrls.TryGetValue(item.ProductIdentifier.Identifier, out string? pageUrl);
    ...
    public async Task<IActionResult> HandleCouponCode(string couponCode, string action, string languageName, CancellationToken cancellationToken)
    {
        if (!string.IsNullOrWhiteSpace(couponCode))
        {
            ...
            else if (string.Equals(action, REMOVE_COUPON_CODE, StringComparison.OrdinalIgnoreCase))
            {
                ...
                string? codeToRemove = shoppingCartData.CouponCodes.FirstOrDefault(c => string.Equals(c, normalizedCode, StringComparison.OrdinalIgnoreCase));
    ...
    private async Task<RedirectResult> RedirectToShoppingCartPage(string languageName, CancellationToken cancellationToken) => Redirect(await webPageUrlProvider.ShoppingCartPageUrl(languageName, cancellationToken));
    ...
    private async Task<string> GetOrderDiscountInfoText(DancingGoatPriceCalculationResult calculationResult, decimal subtotalAfterLineDiscount, ShoppingCartDataModel shoppingCartData, CancellationToken cancellationToken)
    {
        ...
        decimal appliedOrderDiscountAmount = appliedOrderPromotion?.PromotionCandidate.OrderDiscountAmount ?? 0;

        string text = await upsellOrderDiscountService.GetUpsellOrderDiscountMessage(subtotalAfterLineDiscount, appliedOrderDiscountAmount, appliedOrderPromotion?.PromotionID, shoppingCartData.CouponCodes, cancellationToken);
        ...
        if (string.IsNullOrEmpty(text))
        {
            if (appliedOrderPromotion != null)
            {
                ...
                string priceString = priceFormatter.Format(appliedOrderPromotion.PromotionCandidate.OrderDiscountAmount, new PriceFormatContext());
